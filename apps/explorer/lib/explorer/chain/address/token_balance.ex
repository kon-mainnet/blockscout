  ...
  def unfetched_token_balances do
    if BackgroundMigrations.get_tb_token_type_finished() do
      from(
        tb in TokenBalance,
        where:
          ((tb.address_hash != ^@burn_address_hash and tb.token_type == "ERC-721") or tb.token_type == "KONET-20" or
      ...
      from(
        ...
        where:
          ((tb.address_hash != ^@burn_address_hash and t.type == "ERC-721") or t.type == "KONET-20" or
             t.type == "ERC-1155") and
            (is_nil(tb.value_fetched_at) or is_nil(tb.value))
      )
    end
